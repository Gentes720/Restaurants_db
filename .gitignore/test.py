...
def insert_data():
    ...
    try:
        ...
        note = int(note)
        b = True
    if (isinstance(id, int) and isinstance(nom, str) and nom != "" and isinstance(localisation, str) 
        and localisation != "" and isinstance(type, str) and isinstance(note, int) and b):
        ...
        messagebox.showerror('Erreur entrer de bonnes informations')
        ligne_id.delete(0, tk.END)
        ligne_nom.delete(0, tk.END)
        ligne_loc.delete(0, tk.END)
        ligne_type.delete(0, tk.END)
